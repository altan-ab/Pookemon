  ...
  function updatePosition() {
    setPookachu((prev) => {
      ...
      if (prev.direction === 'up') {
        newY = prev.yPosition - 1 // Yukarı
      } else if (prev.direction === 'down') {
        newY = prev.yPosition + 1 // Aşağı
      } else if (prev.direction === 'left') {
        newX = prev.xPosition - 1 // Sola
      } else if (prev.direction === 'right') {
        newX = prev.xPosition + 1 // Sağa
      }

      // Yeni pozisyonu sınırlar içinde tutma
      if (newX < boundaries.xAxis.min) newX = boundaries.xAxis.min
      if (newX > boundaries.xAxis.max) newX = boundaries.xAxis.max
      if (newY < boundaries.yAxis.min) newY = boundaries.yAxis.min
      if (newY > boundaries.yAxis.max) newY = boundaries.yAxis.max

      // Pookachu state'ini güncelle
      return {
        ...prev,
        xPosition: newX,
        yPosition: newY,
